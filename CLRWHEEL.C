...
static char *MODID = "@(#)clrwheel.c:2.01";
...
#pragma	subtitle("   Colour Wheel - Control Window Bitmap Image Procedure")
#pragma	page( )
...
if ( pcw->cPlanes == 0 )
   ...
   DosAllocMem((PPVOID)&plFormats, 2UL * cFormats,
   ...
   pcw->cPlanes	  = plFormats[0];
   pcw->cBitCount = plFormats[1];
...
bminfo2.cx	  = pcw->cx;
bminfo2.cy	  = pcw->cy;
bminfo2.cPlanes	  = pcw->cPlanes;
bminfo2.cBitCount = pcw->cBitCount;
...
if ( pcw->hbm =	hbmCreateBitmap(pcw) )
   {
   pcw->hbmConstruct = 0L;
...
/************************************************************************/
   ...
   case	CWM_SETHSBCLR :
       ...
       lAngle  = LONGFROMMP(mp1);
       lRadius = LONGFROMMP(mp2);
